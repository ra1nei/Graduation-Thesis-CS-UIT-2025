...
#figure(
    ...
    caption: [Kiến trúc mạng DG-Font. Mô-đun FDSC đóng vai trò nòng cốt trong việc học biến dạng hình học giữa các ký tự. (Trích dẫn từ @Xie2021DGFont)]
  )
...
#figure(
  ...
  caption: [Minh hoạ cơ chế Content Fusion: Các đặc trưng từ tập font cơ sở (Source) được tổ hợp tuyến tính dựa trên bộ trọng số dự đoán (Weights) để xấp xỉ cấu trúc hình học của font mục tiêu. (Trích dẫn từ @Wang2023CFFont)]
)
...
#figure( 
  ...
  caption: [Kiến trúc mạng DFS với thành phần cốt lõi là Ma trận Tương đồng (SM) giúp điều hướng dòng chảy thông tin phong cách. (Trích dẫn từ @Zhu2020FewShotTextStyle)] 
)
...
#figure( 
  ...
  caption: [Tổng quan kiến trúc FTransGAN. (Trích dẫn từ @Li2021FTransGAN)]
)
...
#figure(
  ...
  caption: [Quá trình Khuếch tán xuôi. (Trích dẫn từ @geeksforgeeks_diffusion)]
)
...
#figure(
  ...
  caption: [Quá trình Khuếch tán ngược.  (Trích dẫn từ @geeksforgeeks_diffusion)]
)
...
Khái niệm "phong cách" (style) được định nghĩa là tập hợp các đặc trưng thị giác phi cấu trúc và bất biến về mặt ngữ nghĩa, bao gồm các thuộc tính hình học cục bộ như độ dày nét (stroke weight), kiểu dáng chân chữ (serif/sans-serif), độ tương phản nét (stroke contrast), cũng như các đặc tính bề mặt như độ xước hay kết cấu mực in. Khác với "nội dung" (content) quy định cấu trúc tô-pô học để nhận diện ký tự, phong cách đóng vai trò là lớp vỏ thẩm mỹ nhất quán xuyên suốt toàn bộ bộ chữ, đòi hỏi mô hình phải có khả năng tách biệt và tái áp dụng các đặc trưng này lên các khung xương cấu trúc hoàn toàn khác biệt mà không làm thay đổi định danh của ký tự đó.

Để chuyển hoá các khái niệm phong cách thị giác mang tính định tính này thành tín hiệu số học mà máy tính có thể xử lý, bài toán thường tiếp cận theo cơ chế ánh xạ đa tầng thông qua các hàm mất mát. Cụ thể, các thuộc tính vật lý cấp thấp như độ dày nét hay cấu trúc hình học được định lượng gián tiếp thông qua sai số điểm ảnh (L1 Loss) và độ tương đồng cấu trúc (SSIM); ví dụ, sự chênh lệch mật độ điểm ảnh giữa nét dày và nét mảnh sẽ tạo ra giá trị sai số lớn, buộc mô hình phải điều chỉnh trọng số để khớp độ dày. Trong khi đó, các thuộc tính trừu tượng cấp cao như kết cấu bề mặt (texture) hay độ xước bút pháp được định lượng thông qua khoảng cách trong không gian đặc trưng sâu (Deep Feature Space) của các mạng nơ-ron tích chập (như VGG). Tại đây, các bộ lọc tích chập đóng vai trò như các "cảm biến" phát hiện hoạ tiết, và độ tương đồng Cô-sin giữa các vector đặc trưng chính là "thước đo" mức độ khớp về phong cách nghệ thuật.
