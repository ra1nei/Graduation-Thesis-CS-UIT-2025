...
Khoá luận tập trung vào bài toán chuyển đổi xuyên ngôn ngữ thay vì các tác vụ đơn ngữ (như Latin sang Latin hay Hán tự sang Hán tự) bởi lẽ bài toán đơn ngữ hiện nay đã đạt được trạng thái bão hoà với nhiều giải pháp tối ưu, trong khi bài toán đa ngôn ngữ vẫn tồn tại một "khoảng cách miền" (domain gap) lớn chưa được giải quyết triệt để. Việc chuyển đổi giữa hai hệ chữ có cấu trúc đối lập như Latin và Hán tự đặt ra những thách thức khoa học cao hơn về khả năng tách biệt (disentanglement) và suy diễn phong cách bất biến, từ đó chứng minh được tính ưu việt và khả năng tổng quát hoá của thuật toán đề xuất so với việc chỉ hoạt động trong một không gian hình thái đóng kín.
...
Mặc dù việc cung cấp nhiều mẫu tham chiếu (few-shot hoặc full-shot) về lý thuyết sẽ cung cấp cho mô hình nhiều thông tin hơn để tái tạo phong cách chính xác, nhưng cách tiếp cận One-shot (chỉ sử dụng một ảnh mẫu duy nhất) được lựa chọn nhằm tối đa hoá tính ứng dụng thực tiễn và giảm thiểu chi phí lao động cho nhà thiết kế. Trong các kịch bản thực tế, việc yêu cầu người dùng phải vẽ thủ công một lượng lớn ký tự mẫu để khởi chạy hệ thống sẽ đi ngược lại mục tiêu tự động hoá của bài toán sinh phông chữ; do đó, thách thức và đóng góp chính của khoá luận nằm ở việc xây dựng một mô hình đủ mạnh để trích xuất và tổng quát hoá phong cách phức tạp chỉ từ lượng thông tin đầu vào cực tiểu.

_*Phạm vi về dữ liệu*_: Sử dụng *các bộ dữ liệu phông chữ mã nguồn mở hỗ trợ đồng thời cả hai bảng mã*, ví dụ như các bộ font thuộc dự án Google Noto CJK hoặc các font nghệ thuật song ngữ. Việc lựa chọn các nguồn dữ liệu song ngữ này không chỉ đảm bảo tính đa dạng về phong cách mà còn đóng vai trò then chốt trong việc thiết lập cơ chế giám sát thực nghiệm: chúng cung cấp các cặp dữ liệu đối chứng (Paired Ground Truth) chính xác — tức cùng một bộ font luôn tồn tại song song cả ký tự Latin và Hán tự tương ứng. Điều này cho phép mô hình tính toán được các hàm mất mát tái tạo (Reconstruction Loss) một cách tin cậy, khắc phục hạn chế của các phương pháp học không giám sát trên dữ liệu không ghép cặp (unpaired data).

Hơn nữa, việc lựa chọn miền dữ liệu này tuân thủ nghiêm ngặt các thiết lập thực nghiệm chuẩn (Standardized Experimental Settings) đã được xác lập bởi các công trình tiên phong như FontDiffuser@Yang2024FontDiffuser, DG-Font@Xie2021DGFont, và FTransGAN@Li2021FTransGAN. Trong nghiên cứu so sánh, việc đồng bộ hoá dữ liệu đầu vào với các phương pháp SOTA (State-of-the-art) là điều kiện tiên quyết để đảm bảo tính công bằng và trực diện (fair and direct comparison) khi đánh giá hiệu năng. Việc kế thừa hệ quy chiếu dữ liệu này giúp loại bỏ các yếu tố nhiễu do sự khác biệt về phân phối dữ liệu, từ đó khẳng định rằng mọi sự cải thiện về chỉ số định lượng (như FID hay LPIPS) đều phản ánh chính xác sự ưu việt của thuật toán đề xuất thay vì lợi thế của dữ liệu.
...
#tab_eq[
  ...
  *@ketluan – Kết luận và Hướng phát triển*.
  #parbreak()
  Tóm tắt toàn bộ đóng góp chính của khoá luận, bao gồm việc tái hiện pipeline FontDiffuser và đề xuất CL-SCR cho cross-lingual font generation.
  Đề xuất các hướng nghiên cứu mở rộng, như mở rộng sang nhiều ngôn ngữ hơn (tiếng Việt, tiếng Nhật, tiếng Ả Rập), và áp dụng parameter-efficient fine-tuning (như LoRA@Hu2022LoRA hoặc Adapter@Houlsby2019Adapter) để tối ưu tài nguyên huấn luyện.
