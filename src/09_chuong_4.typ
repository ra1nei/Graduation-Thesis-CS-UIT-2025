...
Để đảm bảo tính khách quan và khả năng so sánh công bằng với các nghiên cứu trước đó, khóa luận không tự xây dựng dữ liệu mới mà kế thừa và sử dụng bộ dữ liệu chuẩn từ công trình "Few-shot Font Style Transfer between Different Languages" (Men et al., IEEE). Đây là một tập dữ liệu quy mô lớn được thiết kế chuyên biệt cho bài toán đa ngôn ngữ, bao gồm tổng cộng *818 bộ phông chữ* với sự đa dạng cao về phong cách thiết kế, trải dài từ các loại font cơ bản như serif, sans-serif đến các dạng biến thể phức tạp như thư pháp, viết tay và gothic.
...
Để đánh giá toàn diện khả năng của mô hình, khoá luận thiết lập hai kịch bản kiểm thử với độ khó tăng dần (theo chuẩn của FontDiffuser và DG-Font):
...
==== Tiêu chí đánh giá
Thay vì chấm điểm phức tạp, người tham gia được yêu cầu thực hiện đánh giá dựa trên *lựa chọn ưu tiên*. Cụ thể, với mỗi bộ mẫu, tình nguyện viên cần chọn ra một bức ảnh duy nhất mà họ cho là tốt nhất dựa trên sự cân bằng giữa hai tiêu chí cốt lõi. Thứ nhất là *Tính nhất quán phong cách (Style Consistency)*, tức ảnh sinh ra phải kế thừa chính xác các đặc trưng thị giác của ảnh phong cách (như độ đậm nhạt, kết cấu nét cọ, hoặc kiểu chân chữ serif/sans-serif). Thứ hai là *Tính toàn vẹn nội dung (Content Legibility)*, tức ký tự sinh ra phải duy trì đúng cấu trúc hình học của ảnh nội dung, đảm bảo tính dễ đọc và không bị biến dạng kỳ quái (ví dụ: chữ `丘` trong kịch bản `e2c` phải giữ nguyên các nét ngang dọc đặc trưng, không bị lai tạp thành ký tự Latin). Kết quả cuối cùng được định lượng thông qua *Tỷ lệ Ưu tiên*, tính bằng phần trăm số phiếu bầu chọn mà mỗi mô hình nhận được trên tổng số lượt đánh giá.

== Kết quả Thực nghiệm và Thảo luận
Trong chương này, khoá luận trình bày toàn bộ kết quả thực nghiệm của mô hình đề xuất. Nội dung bao gồm đánh giá định lượng và định tính chi tiết, nghiên cứu bóc tách (ablation study) về các thành phần kiến trúc, khảo sát người dùng, và phân tích các trường hợp thất bại. Các kết quả này được đối chiếu trực tiếp với nhiều mô hình sinh font hiện đại, bao gồm các mô hình GAN-based (DG-Font, CF-Font, FTransGAN), mô hình diffusion-based (DFS), và các phiên bản mô hình của khoá luận.
...
Đối với hướng Hán tự $->$ Latin, khoá luận tiến hành phân loại và đánh giá các kịch bản dựa trên độ phức tạp của ký tự Hán tự được sử dụng làm ảnh tham chiếu phong cách, nhằm phân tích độ nhạy của mô hình đối với sự đa dạng của nét:
...
#figure(
  table(
    ...
    [Medium], [Ảnh phong cách là Hán tự có số nét $11 <= M <= 20$.], [$"Ours"_"Medium"$], [Kiểm tra hiệu quả của các module bảo toàn nét (MCA) khi đối mặt với cấu trúc trung bình.],
...
Việc phân loại theo độ phức tạp này giúp khoá luận xác định module CL-SCR hoặc các kiến trúc lõi khác (MCA, RSI) hoạt động hiệu quả nhất ở mức độ phức tạp cấu trúc nào của phong cách Hán tự, từ đó cung cấp những cái nhìn sâu sắc hơn về khả năng học đặc trưng của mô hình khuếch tán.
...
// Để kiểm chứng trực quan, khoá luận so sánh ảnh sinh của các mô hình trong Hình .
...
Tuy nhiên, *điểm nhấn quan trọng nhất nằm ở sự so sánh giữa mô-đun _S (SCR gốc)_ và _CL (CL-SCR đề xuất)_*. Kết quả thực nghiệm cho thấy *CL-SCR vượt trội hơn hẳn so với SCR gốc*, đặc biệt là *trong các kịch bản khó (Unseen Font)*.
  - Trong hướng `e2c` (UFSC): *Việc thay thế S bằng CL giúp FID giảm mạnh từ _29.10_ xuống _13.55_*.

  - Trong hướng `c2e` (UFSC): *FID giảm từ _59.58_ xuống _41.11_*.
...
Trong kiến trúc *CL-SCR*, *hàm mất mát InfoNCE* đóng vai trò điều hướng không gian biểu diễn phong cách. khoá luận khảo sát *ba biến thể chiến lược huấn luyện* được định nghĩa trong tham số `loss_mode`:
