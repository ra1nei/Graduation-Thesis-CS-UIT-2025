...
#figure(
  ...
  caption: [Mô hình tổng thể của FontDiffuser gồm 2 giai đoạn: #linebreak() Tái tạo cấu trúc (Trái) và Tinh chỉnh phong cách (Phải). (Trích dẫn từ @Yang2024FontDiffuser)]
)
...
5. Sau khi đi qua một kết nối phần dư (residual connection), một lớp tích chập $1 times 1$ được sử dụng để giảm số lượng kênh, thu được đầu ra $I_"co"$.
...
#figure(
  ...
  caption: [Khối MCA (Multi-scale Content Aggregation). (Trích dẫn từ @Yang2024FontDiffuser)]
)

#figure(
  image("../images/FontDiffuser/multi-scale_content_feature.pdf"),
  caption: [Đặc trưng Content ở các khối khác nhau. (Trích dẫn từ @Yang2024FontDiffuser)]
)
...
#tab_eq(indent: 0em)[
  1. Ảnh tham chiếu $x_s$ trước hết được nhúng bởi bộ mã hoá nội dung $E_c$ để thu các bản đồ cấu trúc (structure maps) $F_s = {f_s^1, f_s^2}$.
...
#figure(
  ...
  caption: [Kiến trúc của mô-đun SCR. (Trích dẫn từ @Yang2024FontDiffuser)]
) <scr_framework>
...
#tab_eq(indent: 3em)[
  ...
  *$v dot v'$*: Phép nhân vô hướng, biểu thị độ tương đồng Cô-sin giữa hai vector (đo mức độ giống nhau về phong cách).
